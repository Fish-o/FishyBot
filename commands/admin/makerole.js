...
exports.run = async (client, message, args) => {
    if(!args[0]){
        return message.channel.send('Please enter a role name')
    }

    let whole_args = args.join(' ').trim();
    let name = whole_args.match(/"(.+)"/i);

    if(!whole_args || !name){
        return message.channel.send('Please enter a role name')
    }
    
    whole_args = whole_args.replace(name[0], '').trim();

    let color = colourNameToHex(whole_args)
    if(color){
...
exports.help = {
    ...
    name:"makerole",
    description: "Create a role with a specified color and without any permisions",
